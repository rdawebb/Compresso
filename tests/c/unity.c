...
/*-----------------------------------------------
 * run_test - Helper function to run a single test
 * Called by auto-generated test runners
 *-----------------------------------------------*/
void run_test(void (*test_func)(void), const char *test_name, unsigned int line_number)
{
    Unity.CurrentTestName = test_name;
    Unity.CurrentTestLineNumber = (UNITY_LINE_TYPE)line_number;
    Unity.NumberOfTests++;
    UNITY_CLR_DETAILS();
    UNITY_EXEC_TIME_START();
    if (TEST_PROTECT())
